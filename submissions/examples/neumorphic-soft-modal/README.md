# Neumorphic Soft Modal

**Category:** Component — Soft UI  
**Type:** Pure CSS  
**Folder:** `submissions/examples/neumorphic-soft-modal/`

---

## Overview

The Neumorphic Soft Modal is a responsive, keyboard-accessible pure CSS modal built specifically for soft UI and tactile layout systems, operating on zero JavaScript overhead.

The component is designed around Neumorphic layout principles where elements match their background colors, using dual light-casting shadows (drop-shadow and highlights) to create realistic visual depth. When opened, the backdrop dims and blurs, while the modal card decel-scales into view.

It features a pure CSS **Focus-Restoring URL-Hash Redirect** to return keyboard outline focus back to the triggering element on close.
...
- **Reduced Motion** — automatically cancels scaling animations and sliding translations under `prefers-reduced-motion: reduce`.
...
```
submissions/examples/neumorphic-soft-modal/
...
| `--soft-modal-scale-start` | `0.94` | Starting scale factor of modal content |
| `--soft-modal-translate-y-start` | `20px` | Starting height offset of modal content |
...
Neumorphism (Soft UI) creates a physical, tactile connection with users by mimicking real-world bevels and surfaces. This component achieves visual depth at less than 3KB of raw CSS code:
