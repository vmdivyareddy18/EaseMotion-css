...
A self-contained EaseMotion-css submission that demonstrates the `inert` pattern using the framework's design tokens.

## Features

- Default `.inert` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="inert inert-md inert-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.inert` and variants, plus layout, dark mode, and reduced motion.
