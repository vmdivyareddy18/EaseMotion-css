...
## Overview
The `alm-sticky-header` provides a polished, fixed-top navigation bar. It leverages GPU-accelerated CSS properties to ensure smooth scrolling while using modern flexbox constraints to prevent layout breakage on both desktop (via text truncation) and mobile (via responsive stacking).

## Features
* **Hardware-Accelerated:** Utilizes `will-change` and `translateZ(0)` for buttery-smooth performance.
...
.alm-nav-logo {
  ...
  min-width: 0; /* CRITICAL: Allows shrinking in flex context */
}
