...
A self-contained EaseMotion-css submission that demonstrates the `user invalid` pattern using the framework's design tokens.

## Features

- Default `.user-invalid` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="user-invalid user-invalid-md user-invalid-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.user-invalid` and variants, plus layout, dark mode, and reduced motion.
