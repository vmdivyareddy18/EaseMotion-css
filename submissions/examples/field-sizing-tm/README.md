...
A self-contained EaseMotion-css submission that demonstrates the `field sizing` pattern using the framework's design tokens.

## Features

- Default `.field-sizing` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="field-sizing field-sizing-md field-sizing-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.field-sizing` and variants, plus layout, dark mode, and reduced motion.
