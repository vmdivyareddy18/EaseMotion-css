...
A self-contained EaseMotion-css submission that demonstrates the `mix blend mode` pattern using the framework's design tokens.

## Features

- Default `.mix-blend-mode` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="mix-blend-mode mix-blend-mode-md mix-blend-mode-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.mix-blend-mode` and variants, plus layout, dark mode, and reduced motion.
