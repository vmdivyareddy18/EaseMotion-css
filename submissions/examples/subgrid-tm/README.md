...
A self-contained EaseMotion-css submission that demonstrates the `subgrid` pattern using the framework's design tokens.

## Features

- Default `.subgrid` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="subgrid subgrid-md subgrid-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.subgrid` and variants, plus layout, dark mode, and reduced motion.
