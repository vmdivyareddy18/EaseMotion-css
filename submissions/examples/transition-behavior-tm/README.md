...
A self-contained EaseMotion-css submission that demonstrates the `transition behavior` pattern using the framework's design tokens.

## Features

- Default `.transition-behavior` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="transition-behavior transition-behavior-md transition-behavior-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.transition-behavior` and variants, plus layout, dark mode, and reduced motion.
