...
A self-contained EaseMotion-css submission that demonstrates the `forced colors` pattern using the framework's design tokens.

## Features

- Default `.forced-colors` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="forced-colors forced-colors-md forced-colors-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.forced-colors` and variants, plus layout, dark mode, and reduced motion.
