# CSS Fade-In Modal — E-Commerce Checkout

## Overview

A production-ready, pure CSS-only fade-in modal system designed for modern
e-commerce checkout flows — order confirmation, coupon codes, shipping
method selection, and payment success.
...
```
css-fade-in-modal-ecommerce-checkout/
├── demo.html   # Full showcase: hero, live demo, 4 modal variations, order summary, docs
...
- **Hero section** introducing the checkout modal with CTA buttons
- **Interactive demo** trigger to open the order confirmation modal
- **Four modal variations** with realistic e-commerce content:
  - Order Confirmation
  - Apply Coupon
  - Shipping Method Selection
  - Payment Successful
- **Order summary section** with product rows, subtotal, discount, and total
- **Fade-in + scale-up entrance** animation, with a matching smooth exit
- **Soft backdrop overlay** with subtle blur
- **White background with green/blue accents**, rounded cards, and soft shadows
- **Fully responsive** across desktop, tablet, and mobile
- No JavaScript, no Bootstrap, no Tailwind, no frameworks

## Accessibility

- **Keyboard accessible** — every trigger, close control, and shipping option is operable with `Tab` + `Enter`
...
- **High contrast** — text, prices, and CTA colors meet WCAG AA contrast ratios
- **Large touch targets** — buttons and shipping rows meet a 44px+ minimum hit area
- **Responsive layout** — product rows, forms, and buttons reflow cleanly down to mobile
...
| `ease-slide-up` | Section headers, cards, order summary, and accessibility tiles |
...
| Variable | Purpose |
|---|---|
| `--modal-duration` | Speed of the fade/scale transition (e.g. `0.35s`) |
...
| `--modal-scale` | Starting scale value the card animates up from (e.g. `0.94`) |
...
#custom-modal {
  ...
  --modal-shadow: 0 30px 60px rgba(5, 150, 105, 0.25);
  --modal-scale: 0.94;
}
```

Each of the four showcase modals (`#confirm-modal`, `#coupon-modal`,
`#shipping-modal`, `#success-modal`) demonstrates a distinct override of
these same five variables — see the `Variant Customizations` section of
`style.css`.

## Responsive Support

- **Desktop** — centered modal card, max-width 400–480px depending on variant
- **Tablet** — fluid width with generous padding retained
- **Mobile (≤640px)** — full-width buttons, stacked coupon input/button, tighter card padding, and a slightly reduced entrance scale for a snappier feel on small screens
