# Magnetic Pull Modal (Interactive Interface Layout)

**Category:** Component — Soft UI  
**Type:** Pure CSS  
**Folder:** `submissions/examples/u-magnetic-pull-modal-34191/`

---

## Overview

The Magnetic Pull Modal is a responsive, keyboard-accessible pure CSS modal featuring a magnetic pull entrance animation with zero JavaScript.

The component simulates a magnetic attraction effect by combining translate, scale and easing transitions. As the modal opens, it appears to be pulled toward the center while the blurred backdrop softly fades in, creating an engaging interaction for modern interface layouts.
...
- **Magnetic Pull Animation** — modal is pulled smoothly toward the center using translate and scale transforms.
- **Reduced Motion** — automatically disables motion for users who prefer reduced motion.
...
```
submissions/examples/u-magnetic-pull-modal-34191/
...
<div id="modal-id" class="soft-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby="modal-body">
  ...
  <div class="soft-modal-content" tabindex="-1">
    ...
    <div class="soft-modal-header">
      <h2 id="modal-title" class="soft-modal-title">Interactive Workspace</h2>
...
| `--soft-modal-scale-start` | `0.82` | Starting scale factor of modal content |
| `--soft-modal-translate-x-start` | `-120px` | Starting horizontal pull distance |
| `--soft-modal-translate-y-start` | `-40px` | Starting vertical pull distance |
...
The Magnetic Pull Modal creates a natural attraction effect that draws user attention toward important actions while maintaining accessibility and responsive behaviour. Motion is achieved entirely with CSS transforms and custom properties without requiring JavaScript.

1. **Zero Bundle Bloat** — visual triggers are handled by the browser's native target rendering, avoiding hydration delays.
2. **Dynamic Customization** — CSS custom properties allow designers to tune pull distance, scale and timing for different interface styles.
