...
A self-contained EaseMotion-css submission that demonstrates the `view transitions` pattern using the framework's design tokens.

## Features

- Default `.view-transitions` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="view-transitions view-transitions-md view-transitions-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.view-transitions` and variants, plus layout, dark mode, and reduced motion.
