...
A self-contained EaseMotion-css submission that demonstrates the `interpolate size` pattern using the framework's design tokens.

## Features

- Default `.interpolate-size` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="interpolate-size interpolate-size-md interpolate-size-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.interpolate-size` and variants, plus layout, dark mode, and reduced motion.
