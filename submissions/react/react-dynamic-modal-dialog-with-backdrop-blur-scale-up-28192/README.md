...
A modular, copy-paste ready React component that provides a beautiful modal dialog with a frosted glass backdrop effect and fluid scale-up entrance animation using **EaseMotion CSS**.

## Files
- `DynamicModalDialog.jsx` – The core React component handling state, event propagation, and body scroll locking.
- `style.css` – Custom EaseMotion CSS styles defining the keyframe animations (`easeModalFadeIn`, `easeModalScaleUp`) and backdrop blur filter.
- `demo.html` – A standalone HTML file demonstrating the modal using Babel and React CDNs.

## Installation
1. Copy the `react-dynamic-modal-dialog-with-backdrop-blur-scale-up-28192` folder into your project's `submissions/react/` directory (or wherever your components live).
...
> **Note:** The PR should only modify files inside `submissions/react/react-dynamic-modal-dialog-with-backdrop-blur-scale-up-28192/`. No changes to `core/` or `components/`.
